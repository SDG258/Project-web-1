...
{
    global $db;
    $stmt = $db->prepare("INSERT INTO `message` (`id`, `fromUserID`, `toUserID`, `content`, `deleteUserID`, `createdAt`) VALUES (NULL, ?, ?, ?, NULL, CURRENT_TIMESTAMP);");
    $stmt->execute(array($fromUserID, $toUserID, $content));
    sendNotificationSendMessage($fromUserID, $toUserID);
}

function loadMessage($fromUserID, $toUserID)
{
    global $db;
    $stmt = $db->prepare("SELECT * FROM `message` WHERE (fromUserID = ? AND toUserID = ?) OR (toUserID = ? AND fromUserID = ?) ORDER BY createdAt ASC");
    $stmt->execute(array($fromUserID, $toUserID, $fromUserID, $toUserID));
    return $stmt->fetchAll(PDO::FETCH_ASSOC);
}

function sendNotificationSendMessage($fromUserId, $toUserId)
{
    global $BASE_URL;
    $user = findUserById($toUserId);
    $currentUser = findUserById($fromUserId);
